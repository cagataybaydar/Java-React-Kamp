...
public class JobTitlesManager implements JobTİtlesService {
	...
	public Result add(JobTitles jobtitles) {
		if(getJobByTitle(jobtitles.getJob_titles()).getData() != null) {
			return new ErrorResult(jobtitles.getJob_titles()+ "zaten böyle bir pozisyon tanımlı");
		}
	...
	public DataResult<JobTitles> getJobByTitle(String title) {
		this.jobTitlesDao.findByJobTitle(title);
